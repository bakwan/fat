...
class Ad_log extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('Model_ad_log');
        $this->load->library('form_validation');
    }

    public function index()
    {
        $ad_log = $this->Model_ad_log->select_join();

        $data = array(
            'ad_log_data' => $ad_log
        );

        $this->template->load('templateadmin','ad_log/tb_log_list', $data);
    }

    public function read($id) 
    {
        $row = $this->Model_ad_log->get_by_id($id);
        if ($row) {
            $data = array(
		'id' => $row->id,
		'users' => $row->users,
		...
		'create_at' => $row->create_at,
            ...
            $this->template->load('templateadmin','ad_log/tb_log_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('ad_log'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('ad_log/create_action'),
	    'id' => set_value('id'),
        ...
        $this->template->load('templateadmin','ad_log/tb_log_form', $data);
    }
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('ad_log/update_action'),
    ...
    public function update_action() 
    {
		...
		'create_at' => $this->input->post('create_at',TRUE),
            ...
            $this->Model_ad_log->update($this->input->post('id', TRUE), $data);
	...
	foreach ($this->Model_ad_log->get_all() as $data) {
            $kolombody = 0;
    ...
    public function word()
    {
        ...
        $data = array(
            'tb_log_data' => $this->Model_ad_log->get_all(),
            'start' => 0
        );
        
        $this->load->view('ad_log/tb_log_doc',$data);
    }

}

/* End of file Ad_log.php */
/* Location: ./application/controllers/Ad_log.php */
